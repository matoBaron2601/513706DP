...
type UserBlockRepoFixtures = {
    ...
    getByBothIdsResult?: UserBlockDto | undefined; // Renamed in fake class below
};
...
class FakeUserBlockRepository implements Partial<UserBlockRepository> {
    ...
    // FIX: Renamed from getByBothIds to match the service call (this.repo.getByUserIdAndBlockId)
    async getByUserIdAndBlockId(
        userId: string,
        blockId: string,
        tx?: Transaction
    ): Promise<UserBlockDto | undefined> {
        this.receivedTxs.getByBothIds = tx; // Keeping the receivedTxs key for simplicity
...
describe('UserBlockService', () => {
    ...
    // getByUserIdAndBlockIdOrUndefined (Updated to reflect repository method name change)

    it('getByUserIdAndBlockIdOrUndefined: returns userBlock when found', async () => {
        const ub = makeUserBlock({ id: 'ubX', userId: 'user1', blockId: 'block1' });

        const ubRepo = new FakeUserBlockRepository({
            getByBothIdsResult: ub
        }) as unknown as UserBlockRepository;
        const userRepo = new FakeUserRepository({}) as unknown as UserRepository;

        const svc = new UserBlockService(ubRepo, userRepo);

        const res = await svc.getByUserIdAndBlockIdOrUndefined({ userId: 'user1', blockId: 'block1' });
        expect(res).toEqual(ub);
    });

    it('getByUserIdAndBlockIdOrUndefined: returns undefined when not found', async () => {
        const ubRepo = new FakeUserBlockRepository({
            getByBothIdsResult: undefined
        }) as unknown as UserBlockRepository;
        const userRepo = new FakeUserRepository({}) as unknown as UserRepository;
    ...
    it('checkUserIsOwnerOfUserBlock: throws NotFoundError when user not found', async () => {
        ...
        await expect(svc.checkUserIsOwnerOfUserBlock('ub1', 'nope@example.com')).rejects.toBeInstanceOf(
            NotFoundError
        );
    });
    ...
    it('checkUserIsOwnerOfUserBlock: throws UnauthorizedError when user is not owner', async () => {
        ...
        await expect(svc.checkUserIsOwnerOfUserBlock('ub1', 'user@example.com')).rejects.toBeInstanceOf(
            UnauthorizedError
        );
    });
    ...
    it('delete: throws NotFoundError when repo.delete returns undefined', async () => {
        ...
        await expect(svc.delete('ub1', 'user@example.com')).rejects.toBeInstanceOf(NotFoundError);
    });
    ...
    // NEW TEST: transaction wiring for getByUserIdAndBlockIdOrUndefined
    it('passes transaction through to repository in getByUserIdAndBlockIdOrUndefined', async () => {
        const ub = makeUserBlock({ id: 'ub1' });
        ...
        await svc.getByUserIdAndBlockIdOrUndefined({ userId: 'u1', blockId: 'b1' }, tx);

        // The receivedTxs key is 'getByBothIds' as defined in the fake repo
        expect(fakeUbRepo.receivedTxs.getByBothIds).toBe(tx);
    });
});