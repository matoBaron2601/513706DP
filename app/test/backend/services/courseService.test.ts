...
describe('CourseService', () => {
	...
	it('delete: throws UnauthorizedError when user is not creator', async () => {
		...
		await expect(svc.delete('course1', 'user@example.com')).rejects.toBeInstanceOf(
			UnauthorizedError
		);
	});
	...
	it('publishCourse: throws ConflictError when no blocks', async () => {
		...
		await expect(svc.publishCourse('course1', 'user@example.com')).rejects.toBeInstanceOf(
			ConflictError
		);
	});
	...
	it('unpublishCourse: throws NotFoundError when update returns undefined', async () => {
		...
		await expect(svc.unpublishCourse('course1', 'user@example.com')).rejects.toBeInstanceOf(
			NotFoundError
		);
	});
