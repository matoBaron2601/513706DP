...
class FakeAdaptiveQuizAnswerRepository implements Partial<AdaptiveQuizAnswerRepository> {
	...
	async getById(id: string, tx?: Transaction): Promise<AdaptiveQuizAnswerDto | undefined> {
	...
	async delete(id: string, tx?: Transaction): Promise<AdaptiveQuizAnswerDto | undefined> {
...
describe('AdaptiveQuizAnswerService', () => {
	...
	it('getByIds: returns array of answers', async () => {
		const rows = [makeAdaptiveQuizAnswer({ id: 'a1' }), makeAdaptiveQuizAnswer({ id: 'a2' })];
