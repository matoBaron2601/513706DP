...
function createAppWithService(service: FakeCourseService) {
	return createCourseApi(service as unknown as CourseService);
}
...
describe('courseApi', () => {
	...
	it('POST /course/filtered calls service.getAll with filter body', async () => {
		const courses: GetCoursesResponse[] = [
			{
				id: 'c1',
				name: 'Course 1',
				creatorId: 'u1',
		...
		const service = new FakeCourseService({ getAll: courses });
		const app = createAppWithService(service);
		...
		const res = await app.handle(
			new Request('http://localhost/course/filtered', {
				method: 'POST',
		...
		expect(res.status).toBe(200);
		const body = await res.json();
		expect(service.calls.getAll.length).toBe(1);
		expect(service.calls.getAll[0]).toEqual(filter);
