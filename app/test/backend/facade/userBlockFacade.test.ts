...
class FakeUserBlockService {
    ...
    calls = {
        // FIX: Renamed to match the method name used in the facade
        getByUserIdAndBlockIdOrUndefined: [] as { data: CreateUserBlock; tx: any }[],
    ...
    // FIX: Implemented the correct method name
    async getByUserIdAndBlockIdOrUndefined(data: CreateUserBlock, tx?: any): Promise<UserBlock | undefined> {
        this.calls.getByUserIdAndBlockIdOrUndefined.push({ data, tx });
...
function makeFacade(deps?: {
    ...
    // Inject the fake services into the facade instance
...
describe('UserBlockFacade getOrCreateUserBlock', () => {
    it('returns existing userBlock if already present', async () => {
        ...
        try {
            ...
            expect(userBlockService.calls.getByUserIdAndBlockIdOrUndefined.length).toBe(1);
    ...
    it('creates userBlock, adaptiveQuiz and conceptProgress when not existing', async () => {
        ...
        try {
            ...
            expect(placementQuizService.calls.getByBlockId[0].tx).toBe(txMock);

            // Assertions for adaptiveQuizService
            ...
            expect(adaptiveQuizService.calls.create[0].tx).toBe(txMock);

            // Assertions for conceptService
            expect(conceptService.calls.getManyByBlockId.length).toBe(1);
            expect(conceptService.calls.getManyByBlockId[0].blockId).toBe('b1');
            expect(conceptService.calls.getManyByBlockId[0].tx).toBe(txMock);

            // Assertions for conceptProgressService
            ...
            expect(conceptProgressService.calls.createMany[0].tx).toBe(txMock);
        } finally {
            (db as any).transaction = originalTransaction;
        }
    });
