...
describe('ConceptFacade - updateCompleteness', () => {
    it('marks concepts mastered when criteria are met', async () => {
        // Calculation check for Criterium 3 (Width):
        // a=160, b=40. a+b=200. a+b+1=201. (a+b)^2=40000.
        // variance = (160 * 40) / (40000 * 201) = 0.000796...
        // sqrt(variance) = 0.02821...
        // width = 2 * 1.96 * 0.02821... = 0.1105... (Met: <= 0.15)
        
        const cp1 = makeConceptProgress({
            id: 'cp1',
            score: 0.8, // C1: score >= 0.8 (MET)
            streak: 5, // C2: streak >= 5 (FIXED: MET)
            alfa: 160,
            beta: 40,
            askedA1: 5, // C4: asked >= 5 (MET by total asked)
            askedA2: 0,
            askedB1: 0,
            askedB2: 0
        });
...
describe('ConceptFacade - updateConceptProgress', () => {
    it('aggregates answers per conceptProgress and updates stats', async () => {
        ...
        // streak = 1 (since the last answer was correct)
