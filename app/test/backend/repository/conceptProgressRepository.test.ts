...
function makeConceptProgress(overrides: Partial<ConceptProgressDto> = {}): ConceptProgressDto {
	return {
		id: 'cp1',
		userBlockId: 'ub1',
		conceptId: 'c1',
...
describe('ConceptProgressRepository', () => {
	...
	it('getById: returns undefined when not found', async () => {
		const repo = new ConceptProgressRepository(makeFakeDbClient({ selectResult: [] }).getDbClient);
	...
	it('update: returns undefined when nothing updated', async () => {
		...
		const repo = new ConceptProgressRepository(makeFakeDbClient({ updateReturn: [] }).getDbClient);
	...
	it('delete: returns undefined when nothing deleted', async () => {
		const repo = new ConceptProgressRepository(makeFakeDbClient({ deleteReturn: [] }).getDbClient);
