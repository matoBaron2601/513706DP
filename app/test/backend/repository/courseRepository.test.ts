...
type Fixtures = {
    ...
    // Removed getAllResult as it's not needed for the current CourseRepository implementation
};
...
describe('CourseRepository', () => {
    ...
    // getAvailableCourses (Fix: Renamed from 'getAll' and updated to use CourseDto)

    it('getAvailableCourses: returns available courses', async () => {
        const rows: CourseDto[] = [
            makeCourse({ id: 'c1', name: 'Published Course', published: true }),
            makeCourse({ id: 'c2', name: 'Own Draft', creatorId: 'u1', published: false })
        ];
        
        // This simulates the result of the select().from().where() in getAvailableCourses
        const repo = new CourseRepository(makeFakeDbClient({ selectResult: rows }).getDbClient);

        const res = await repo.getAvailableCourses('u1');
        expect(res).toEqual(rows);
    });

    it('getAvailableCourses: returns empty array when nothing found', async () => {
        const repo = new CourseRepository(makeFakeDbClient({ selectResult: [] }).getDbClient);
    ...
    it('uses provided transaction when calling getDbClient in getAvailableCourses', async () => {
        const rows: CourseDto[] = [makeCourse({ id: 'c1', published: true })];
        ...
        await repo.getAvailableCourses('u1', tx);
