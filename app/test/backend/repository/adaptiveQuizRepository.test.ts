		...
		orderBy(_expr: unknown) {
			...
			return {
				...
				then<TResult1 = AdaptiveQuizDto[], TResult2 = never>(
					onfulfilled?: ((value: AdaptiveQuizDto[]) => TResult1 | PromiseLike<TResult1>) | null,
...
describe('AdaptiveQuizRepository', () => {
	...
	it('getById: returns adaptiveQuiz when it exists', async () => {
		...
		const repo = new AdaptiveQuizRepository(makeFakeDbClient({ selectResult: [row] }).getDbClient);
	...
	it('getById: returns undefined when not found', async () => {
		const repo = new AdaptiveQuizRepository(makeFakeDbClient({ selectResult: [] }).getDbClient);
	...
	it('getByIds: returns array of rows', async () => {
		...
		const repo = new AdaptiveQuizRepository(makeFakeDbClient({ selectResult: rows }).getDbClient);
	...
	it('update: returns undefined when returning() is empty', async () => {
		...
		const repo = new AdaptiveQuizRepository(makeFakeDbClient({ updateReturn: [] }).getDbClient);
	...
	it('delete: returns undefined when nothing was deleted', async () => {
		const repo = new AdaptiveQuizRepository(makeFakeDbClient({ deleteReturn: [] }).getDbClient);
