		...
		insert(_table: unknown) {
			return {
				values(_v: CreateBaseQuestionDto | CreateBaseQuestionDto[]) {
					return {
						returning: () =>
							Promise.resolve(fixtures.insertReturn ?? fixtures.createManyReturn ?? [])
					};
				}
			};
		},
...
describe('BaseQuestionRepository', () => {
	...
	it('getById: returns question when found', async () => {
		...
		const repo = new BaseQuestionRepository(makeFakeDbClient({ selectResult: [row] }).getDbClient);
	...
	it('getById: returns undefined when not found', async () => {
		const repo = new BaseQuestionRepository(makeFakeDbClient({ selectResult: [] }).getDbClient);
	...
	it('update: returns undefined when nothing updated', async () => {
		const repo = new BaseQuestionRepository(makeFakeDbClient({ updateReturn: [] }).getDbClient);
	...
	it('deleteById: returns undefined when nothing deleted', async () => {
		const repo = new BaseQuestionRepository(makeFakeDbClient({ deleteReturn: [] }).getDbClient);
	...
	it('getByIds: returns matching rows', async () => {
		...
		const repo = new BaseQuestionRepository(makeFakeDbClient({ selectResult: rows }).getDbClient);
